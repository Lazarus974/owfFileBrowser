...
<li class="list-group-item files-node" data-level="%{$tree['level']}%" data-parent="%{$tree['parent']}%">
	%{if $tree['type'] == 'f'}%
...
%{if $tree['type'] == 'd'}%
	<li class="list-group-item list-group-item-info files-node" data-level="%{$tree['level']+1}%" data-parent="%{$tree['parent']}%%{$tree['name']}%/">
